...
const resetSelectedFacetsComponent = (facet, type) => {
  ...
  return (
    <div>
      ...
      <div
        className={`fixed inset-0 bg-lime-400 bg-opacity-30 z-40 lg:hidden lg:z-auto transition-opacity duration-200 ${
      ...
      <div
        id="sidebar"
        ref={sidebar}
        className={`flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0 bg-lime-300 p-4 transition-all duration-200 ease-in-out ${
      ...
      >
        ...
        <div className="flex justify-between mb-10 pr-3 sm:px-2">
          ...
          <button
            ref={trigger}
            className="lg:hidden bg-lime-400 hover:text-slate-700"
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            <SidebarLinkGroup activecondition={context.searchResponse}>
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      <a
                        ...
                        className={`block text-slate-900 truncate transition duration-150 ${
                          context.searchResponse ? 'hover:text-slate-900' : 'hover:text-white'
                        }`}
                        onClick={(e) => {
                          e.preventDefault();
                      ...
                      >
                        <div className="flex items-center justify-between">
                            <span className="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Facets
                            </span>
                          {/* Icon */}
                      ...
                      <SelectedFacets className={`pl-9 mt-1 ${!open && 'hidden'}`}
                          ...
                          separator="to"
                        ></SelectedFacets>
                        <div class="flex-none m-5" className={`pl-9 mt-1 ${!open && 'hidden'}`}>
                          {context.searchResponse?.facets?.Categorie && <StringFacet containerStyle={{'background-color': 'blue'}} title="Categorie" name="Categorie" data={context.searchResponse?.facets?.Categorie} onSelect={handleFacetClick} reset={resetStringFacet} />}
                          {context.searchResponse?.facets?.Materiau && <StringFacet containerStyle={{'background-color': 'none'}} title="Materiau" name="Materiau" data={context.searchResponse?.facets?.Materiau} onSelect={handleFacetClick} reset={resetStringFacet} />}
                          {context.searchResponse?.facets?.Type && <StringFacet containerStyle={{'background-color': 'none'}} title="Type" name="Type" data={context.searchResponse?.facets?.Type} onSelect={handleFacetClick} reset={resetStringFacet} />}
                            ...
                            <svg className="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                              <path
                                className={`fill-current ${
                                  pathname === '/' || pathname.includes('dashboard') ? 'text-indigo-500' : 'text-slate-700'
                                }`}
                              ...
                              <path
                                className={`fill-current ${
                                  pathname === '/' || pathname.includes('dashboard') ? 'text-indigo-200' : 'text-slate-700'
                                }`}
                        ...
                        <ul className={`pl-9 mt-1 ${!open && 'hidden'}`}>
                          <li className="mb-1 last:mb-0">
                            <NavLink
                              ...
                              to="/"
                              className={({ isActive }) =>
                                'block transition duration-150 truncate ' + (isActive ? 'text-indigo-500' : 'text-slate-700 hover:text-slate-900')
                              }
                            >
